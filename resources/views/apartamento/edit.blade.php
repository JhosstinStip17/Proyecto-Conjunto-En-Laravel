...
@section('content')
<div class="container">
    <h1 class="text-center">Editar un Apartamento</h1>
    ...
    <form action="{{route('apartamento.update', $apartamento)}}" method="post">
        ...
        <div class="input-group mb-3">
            ...
            <input type="text" name="nombre" value="{{$apartamento->nombre}}" class="form-control"
                aria-label="Sizing example input" aria-describedby="inputGroup-sizing-default" required>
        </div>
        <label for="torre_id">Torre</label>
        <select class="form-select" aria-label="Default select example" name="torre_id" id="torre_id" onclick="updateConjuntos()">
            @foreach ($torres as $torre)
                <option value="{{$torre->id}}" {{$apartamento->torre_id == $torre->id ? 'selected' : '' }}>
                    {{$torre->nombre}} - {{{$torre->conjunto->nombre}}}
                </option>
            @endforeach
        </select>
